...
function snapshotArray(
  ...
  if (context.arraySlots > MAX_TOTAL_ARRAY_SLOTS) return invalidDto();

  const keys = Reflect.ownKeys(value);
  if (keys.length !== length + 1) return invalidDto();
...
function snapshotObject(
  ...
  const keys = Reflect.ownKeys(value);
  if (keys.length > MAX_OBJECT_PROPERTIES) return invalidDto();
  context.propertyCount += keys.length;
  if (context.propertyCount > MAX_TOTAL_PROPERTIES) return invalidDto();
...
function snapshotValue(
  ...
  if (typeof value === 'string') return snapshotString(value, context);
