...
function findEocd(view: DataView): number {
  if (view.byteLength < EOCD_SIZE) {
    malformed('ZIP end-of-central-directory record is missing.');
  }
  ...
  return malformed('ZIP end-of-central-directory record is missing.');
}
...
export function isZipArchiveCandidate(data: ArrayBuffer | Uint8Array): boolean {
  ...
  const minimumOffset = Math.max(0, bytes.byteLength - EOCD_SIZE - MAX_COMMENT_BYTES);
  for (let offset = bytes.byteLength - EOCD_SIZE; offset >= minimumOffset; offset -= 1) {
    if (view.getUint32(offset, true) === EOCD_SIGNATURE) {
      return true;
    }
  }
...
function decodeName(nameBytes: Uint8Array, flags: number): string {
  try {
    const encoding = (flags & UTF8_FLAG) !== 0 ? 'utf-8' : 'windows-1252';
    return new TextDecoder(encoding, { fatal: true }).decode(nameBytes);
  } catch (error) {
