...
export function EquityPage() {
  ...
  const [esop, setEsop] = useState(() => localStorage.getItem('dd-esop') || '10');
  const [invest, setInvest] = useState(() => localStorage.getItem('dd-invest') || '');
  ...
  return (
    <div className="module-page"><h1>股权与融资</h1>
      <form className="module-form" onSubmit={e => e.preventDefault()}>
        <label>ESOP Pool %<input value={esop} onChange={e => { setEsop(e.target.value); localStorage.setItem('dd-esop', e.target.value); }} /></label>
        <label>Investment Amount<input value={invest} onChange={e => { setInvest(e.target.value); localStorage.setItem('dd-invest', e.target.value); }} /></label>
      </form>
      <h2>Cap Table</h2>
