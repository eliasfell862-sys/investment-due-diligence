...
export function ReportExportPage() {
  ...
  return (
    <div className="module-page" style={{maxWidth:960}}>
      ...
      <h1>投资尽调报告</h1>
      <button className="button" onClick={() => { const pid = projectId || 'default'; localStorage.removeItem(`dd-p-${pid}-quality`); localStorage.removeItem(`dd-p-${pid}-risk-items`); setRefreshKey(k => k + 1); }} style={{marginBottom:16,color:'#fff',borderColor:'var(--teal)',background:'var(--teal)'}}>🔄 重新分析计算</button>
